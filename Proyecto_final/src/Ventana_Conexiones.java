...
public class Ventana_Conexiones {
	...
	//			frmStockManager.setVisible(true);
				try {
					...
					// INITIALIZE THREAD (TEXTAREA FROM ALMACEN MISSING)
	...
	//				t.start();
		...
		btnConectar_Almacen.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					frm_almacen.setVisible(true);
				client = new Socket(SERVER, PORT);// open socket
				...
				ReadingInput t = new ReadingInput(input, textArea2);
				t.start();
